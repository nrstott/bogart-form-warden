...
// This function converts function-objects to strings with a prefix of 'FUNCTION:'.
// In essence, it serializes a function.
function funcStringify(validationOptions) {
  for (var k in validationOptions.fields) {
    var factors = validationOptions.fields[k];
    factors.forEach(function (optionObj) {
      // Watch for function-objects.
      if (typeof optionObj.isValid === 'function') {
        // Convert function to a string with special prefix.
        optionObj.isValid = "FUNCTION:" + optionObj.isValid.toString();
      }
    });
  }
  ...
  return function (viewEngine, opts) {
    opts.locals.formWardenScript =
      ...
      util.format('<script type="text/javascript">' +
          ...
          '</script>',
        selector,
        // Note: Convert functions to strings prior to JSON.stringify.
        JSON.stringify(funcStringify(validationOptions)));
  };
}
...
function readFormwardenScript() {
  return '<script type="text/javascript">' +
    formWarden.getFormWardenScriptContents() + '\n\n' +
    formWarden.getJqueryFormWardenScriptContents() +
    '</script>';
}
